...
def generate(mode: str) -> int:
    ...
    # До 3 попыток. Список 8 слов форсирован в user_message, поэтому
    # эскалация нужна только на содержательные ошибки: MC-дубли,
    # matching-путаницы, опечатки, session-lock, сеть. 3 попытки нужны
    # потому что модель на разных попытках делает РАЗНЫЕ ошибки —
    # исправит одну, сломает другую.
    ...
    for attempt in (1, 2, 3):
        extra = ""
        if attempt >= 2:
            urgency = "⚠️" if attempt == 2 else "🚨 ФИНАЛЬНАЯ ПОПЫТКА"
            extra = (
                f"{urgency} Предыдущая попытка отклонена валидатором:\n\n"
                f"{last_error}\n\n"
                f"Сгенерируй заново, целиком. Исправь конкретно "
                f"ту ошибку, что указана выше. Не сломай другие требования: "
                f"ЗАДАННЫЙ СПИСОК 8 слов остаётся тот же, MC-варианты "
                f"уникальны, matching-пары семантически верны, правильные "
                f"ответы без опечаток."
            )
        try:
    ...
    if manifest is None:
        common.send_message(
            f"Тренировка {today}: после 3 попыток модель так и не дала "
